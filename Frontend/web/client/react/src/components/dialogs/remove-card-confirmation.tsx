...
interface RemoveCardConfirmationProps {
  openRemoveCardDialog: boolean;
  setRemoveCardDialog: (open: boolean) => void;
  onConfirmRemove: () => void;
  isLoading?: boolean;
}

const RemoveCardConfirmation = (props: RemoveCardConfirmationProps) => {
  const handleDialogClose: DialogProps["onClose"] = (event, reason) => {
    if (reason !== "backdropClick") {
      props.setRemoveCardDialog(false);
    }
  };
  ...
  return (
    <>
      <Dialog
        open={props.openRemoveCardDialog}
          ...
          <div className="md:w-md xl:w-md lg:w-md xs:w-sm sm:w-sm w-sm">
            <div className="flex justify-start">
              <span
                ...
                onClick={() => props.setRemoveCardDialog(false)}
              >
            ...
            <div>
              <div className="flex justify-center">
                <div className="flex h-[64px] w-[64px] items-center justify-center rounded-[12px] bg-[#FFECDF]">
                  <i className="ri-delete-bin-line text-[32px] text-[#CC1A30]"></i>
                </div>
              </div>
