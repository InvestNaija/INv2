...
class Calculator {
    static compounding(initialAmt = 0, PMT, frequency, interestRate, startDate, endDate) {
        ...
        const futureValue = Math.round((((P * Math.pow(1 + (r / n), n * t) + (PMT * ((Math.pow(1 + (r / n), n * t) - 1) / (r / n))))) + Number.EPSILON) * 100) / 100;
    ...
    ;
    static plan(initialAmt = 0, futureValue, frequency, interestRate, startDate, endDate) {
        const { P, t, n, r } = Calculator.initialize(initialAmt, frequency, interestRate, startDate, endDate);
    ...
    ;
    static once(PMT, frequency, interestRate, startDate, endDate) {
    ...
    ;
    static initialize(P, frequency, interestRate, startDate, endDate) {
        const t = Number(((0, moment_1.default)(endDate).diff(startDate, 'days') / 365).toFixed(2)); //tenure in years
        const n = Calculator.getFrequency(frequency);
        const r = interestRate / 100;
        return { P, t, n, r };
    }
    ;
    static getFrequency(strFreq) {
        let numFreq = 1;
