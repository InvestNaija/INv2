...
export class UserUpdatedListener extends Listener<UserUpdatedEvent> {
   ...
   async onMessage(data: UserUpdatedEvent['data'], channel: Channel, msg: Message): Promise<void> {
      try {
         ...
         console.log('======> Finance received user updated with id: ', data.user!.id,);

         const user = await this.userRepo.findOne([], {id: data.user!.id, version: data.user!.version-1});
